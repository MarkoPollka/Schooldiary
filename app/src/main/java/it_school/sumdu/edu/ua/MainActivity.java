...
// вставляем запись и получаем ее ID
                ...
                Toast toast = Toast.makeText(getApplicationContext(),
                        "Новое домашнее задание добавлено в дневник",
                        Toast.LENGTH_SHORT);
                toast.show();
                txtSubject.getText().clear();
                txtDate.getText().clear();
                txtTask.getText().clear(); break;
            case R.id.button2:
                List<String> subjectlist = new ArrayList<String>(); List<String> tasklist= new
                    ArrayList<String>();
                String dateQuery = txtDateQuery.getText().toString(); String sqlQuery = "select * "
                    + "from " + TABLE_DIARY
                    + " where date = ?";
                ...
                if ((subjectlist.isEmpty())&&(tasklist.isEmpty()))
                {
                    Toast t = Toast.makeText(getApplicationContext(),
                            "На эту дату домашнего задания нет", Toast.LENGTH_LONG);
                    t.show();
                }else {
