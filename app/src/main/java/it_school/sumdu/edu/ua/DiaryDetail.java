...
public class DiaryDetail extends AppCompatActivity {
    ListView listView; CustomAdapter
            adapter;
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.homework_list);
        listView = (ListView)findViewById(R.id.listView); ArrayList<String>
                subjectlist =
                getIntent().getStringArrayListExtra("subjectlist");
...
// Использование собственного шаблона
        adapter = new CustomAdapter(getApplicationContext(),
                R.layout.homework_list_row);
        listView.setAdapter(adapter); int i = 0;
